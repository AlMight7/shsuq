...
async def get_top_animes(gnr: str, page, user):
    ...
    if int(page)==1:
        if int(data['pageInfo']['lastPage'])!=1:
            btn.append([InlineKeyboardButton("Next ➡️", callback_data=f"topanimu_{gnr}_{int(page)+1}_{user}")])
    elif int(page) == int(data['pageInfo']['lastPage']):
        btn.append([InlineKeyboardButton("⬅️ Prev", callback_data=f"topanimu_{gnr}_{int(page)-1}_{user}")])
    else:
        btn.append([
            InlineKeyboardButton("⬅️ Prev", callback_data=f"topanimu_{gnr}_{int(page)-1}_{user}"),
            InlineKeyboardButton("Next ➡️", callback_data=f"topanimu_{gnr}_{int(page)+1}_{user}")
        ])
...
async def get_user_favourites(id_, user, req, page, sighs):
    ...
    if int(page)==1:
        if int(data['pageInfo']['lastPage'])!=1:
            btn.append([InlineKeyboardButton("Next ➡️", callback_data=f"myfavqry_{req}_{id_}_{str(int(page)+1)}_{sighs}_{user}")])
    elif int(page) == int(data['pageInfo']['lastPage']):
        btn.append([InlineKeyboardButton("⬅️ Prev", callback_data=f"myfavqry_{req}_{id_}_{str(int(page)-1)}_{sighs}_{user}")])
    else:
        btn.append([
            InlineKeyboardButton("⬅️ Prev", callback_data=f"myfavqry_{req}_{id_}_{str(int(page)-1)}_{sighs}_{user}"),
            InlineKeyboardButton("Next ➡️", callback_data=f"myfavqry_{req}_{id_}_{str(int(page)+1)}_{sighs}_{user}")
        ])
...
Total Volumes Rea d: `{manga['volumesRead']}`
